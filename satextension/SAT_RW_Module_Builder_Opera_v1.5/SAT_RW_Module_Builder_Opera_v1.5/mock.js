...
let D=null, mod=1, qs=[], idx=0, answers={}, flagged={}, eliminated={};
let left=1920, tick=null, paused=false, timerHidden=false, warned5=false;
let elimOn=false, view='question', splitPct=50;
...
function sectionTitle(n){return 'Section 1, Module '+n+': Reading and Writing'}
...
function begin(n){
  mod=n; qs=n===1?D.m1:D.m2; idx=0; left=1920; paused=false; timerHidden=false; warned5=false; elimOn=false; view='question';
...
function renderGrid(){
  ...
  qs.forEach((_,i)=>{
    ...
    b.onclick=()=>{idx=i; closePopovers(); showQuestionView(); render()};
    g.appendChild(b);
  });
}
...
function renderReview(){
  ...
  qs.forEach((_,i)=>{
    ...
    b.onclick=()=>{idx=i; showQuestionView(); render()};
    g.appendChild(b);
  });
...
function finishModule(){
  ...
  if(mod===1) $('#between').classList.remove('hidden');
  else showResults();
}
