...
chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
  ...
  if(msg?.type==='status'){
    chrome.storage.local.get(AUTH_KEY).then(s=>{
      const a=s[AUTH_KEY];
      sendResponse({hasAuth:!!(a?.authenticationToken&&a?.authorizationToken),capturedAt:a?.capturedAt||null,authAgeMs:a?.capturedAt?Date.now()-a.capturedAt:null,capturedFrom:a?.capturedFrom||null,source:a?.source||null});
    });
    return true;
  }
