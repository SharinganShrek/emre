...
chrome.webRequest.onBeforeSendHeaders.addListener(details=>{
  ...
  if(authenticationToken&&authorizationToken){
    chrome.storage.local.set({[AUTH_KEY]:{authenticationToken,authorizationToken,capturedAt:Date.now()}});
  }
},{urls:[API+'/*'],types:['xmlhttprequest']},['requestHeaders','extraHeaders']);

chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
  if(msg?.type==='status'){
    chrome.storage.local.get(AUTH_KEY).then(s=>{
      const a=s[AUTH_KEY];
      sendResponse({hasAuth:!!(a?.authenticationToken&&a?.authorizationToken),capturedAt:a?.capturedAt||null,authAgeMs:a?.capturedAt?Date.now()-a.capturedAt:null});
    });
    return true;
  }
  ...
  if(msg?.type==='buildMock'){
    buildMock(msg.options||{}).then(sendResponse).catch(e=>sendResponse({ok:false,error:e.message})); return true;
  }
});
...
async function apiPost(path,body,auth){
  ...
  throw new Error(`${path} was rejected (HTTP ${r?.status||403}).${detail} Your saved College Board session token has been cleared. Keep the Question Bank tab open, change any filter (for example Easy → Medium → Easy) so the page makes a fresh API request, then reopen the extension.`);
}
...
async function buildMock(options){
  const s=await chrome.storage.local.get([AUTH_KEY,HISTORY_KEY]); const auth=s[AUTH_KEY];
  if(!auth?.authenticationToken||!auth?.authorizationToken) throw new Error('No College Board session captured. Open the Student Question Bank, change any filter once, then retry.');
  if(!auth.capturedAt || Date.now()-auth.capturedAt>2*60*60*1000){
    await chrome.storage.local.remove(AUTH_KEY);
    throw new Error('The captured College Board session is older than 2 hours and may be expired. In the Question Bank tab, change any filter once so a fresh API request is made, then retry.');
  }
  const warnings=[];
  ...
  return {ok:true,module1:m1.length,module2:m2.length,warnings:[...new Set(warnings)]};
}