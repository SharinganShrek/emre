...
chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
  if(msg?.type==='status'){
    chrome.storage.local.get(AUTH_KEY).then(s=>sendResponse({hasAuth:!!(s[AUTH_KEY]?.authenticationToken&&s[AUTH_KEY]?.authorizationToken)}));
    return true;
  }
...
async function apiPost(path,body,auth){
  const r=await fetch(API+path,{method:'POST',headers:{'accept':'application/json, text/plain, */*','content-type':'application/json','x-cb-catapult-authentication-token':auth.authenticationToken,'x-cb-catapult-authorization-token':auth.authorizationToken},body:JSON.stringify(body||{})});
  if(!r.ok) throw new Error(`${path} failed (HTTP ${r.status}). Refresh the Question Bank page and retry.`);
  return r.json();
}
...
async function buildMock(options){
  ...
  if(!auth?.authenticationToken||!auth?.authorizationToken) throw new Error('No College Board session captured. Open/refresh Student Question Bank results first.');
  const warnings=[];
